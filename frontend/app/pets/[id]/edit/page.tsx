...
export default function EditPetPage() {
    ...
    const { user } = useAuth();
    const { pet, isLoading: isPetLoading, updatePet } = usePet(params.id as string);
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <Link href={`/pets/${params.id}`} className="inline-flex items-center gap-2 text-stone-600 hover:text-sage-700 mb-6">
                <ArrowLeft className="w-4 h-4" />
                Volver al perfil
            </Link>

            <div className="bg-white rounded-2xl p-8 border border-sage-200">
                <h1 className="text-2xl font-bold text-sage-900 mb-6">Editar Mascota: {pet?.name}</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex gap-3 pt-4">
                        <Link href={`/pets/${params.id}`} className="flex-1 btn-secondary text-center py-3">
                            Cancelar
                        </Link>
